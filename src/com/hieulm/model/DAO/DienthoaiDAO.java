...
public class DienthoaiDAO {
	...
	public Dienthoai getDienthoaiById(int id){
		...
		try{
			session = HibernateUtil.getSessionFactory().openSession();
			tx=session.beginTransaction();
			dienthoais = session.createQuery("FROM Dienthoai dienthoai where dienthoai.madienthoai = :id")
	...
	public List<Dienthoai> getDienthoaiByHangsx(int hangsxId){
		...
		try{
			session = HibernateUtil.getSessionFactory().openSession();
			tx=session.beginTransaction();
//			List<Hangsx> hangsxs = session.createQuery("From Hangsx hangsx where hangsx.mahangsx = 1").list();
//			Hangsx hangsx = hangsxs.get(0);
			dienthoai = session.createQuery("FROM Dienthoai dienthoai where dienthoai.hangsx.mahangsx = :brandId")
